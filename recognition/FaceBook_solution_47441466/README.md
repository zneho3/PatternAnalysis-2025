...
The model used for this problem is a Graph Neural Network (GNN) implemented using the PyTorch Geometric library. The architecture consists of two graph convolutional layers using a ReLu layer in between. Originally the plan was to use a more complicated architecture with more layers but this simple architecture worked quite well upon initial creation and testing so it was kept. Pytorch Geometric was chosen due to its ease of use and extensive documentation, which made it suitable for implementing GNNs efficiently, and allowed for easy experimentation with different architectures and hyperparameters with lots of abstractions already implemented.

### GCN Architecture

At a high the level, a graph convolutional network (GCN) works by learning patterns from a labelled data point and using its features (which includes information about the node itself as well as its neighbours) to predict the labels of unlabelled data points. The GCN layers aggregate information from a node's local neighbourhood, allowing the model to learn representations that capture both node features and graph structure. The image below shows the architecture of GCN's like the one used in this project. It also shows how embeddings are represented in lower dimensions for visualisation purposes.

![GCN Architecture](gcn.jpg)
...
There is also an implicit softmax operation wrapped around the output during training to convert the logits into probabilities for each class. [This paper](https://arxiv.org/pdf/1609.02907) was used to understand the GCN architecture better.

## Usage Instructions

Download the dataset from the provided link and place it in the Facebook_solution_47441466 folder. Ensure that the file names are not changed from the source, and that the files are formatted as such:

```text
Facebook_solution_47441466
└──facebook
...
This paper was used to understand the GCN architecture and theory better: [SEMI-SUPERVISED CLASSIFICATION WITH
GRAPH CONVOLUTIONAL NETWORKS](https://arxiv.org/pdf/1609.02907)

This tutorial was used to understand implementing GNNs using PyTorch Geometric:
...
The PyTorch Geometric documentation was used extensively for reference on functions and classes and in particular this page: [PyTorch Geometric Documentation](https://pytorch-geometric.readthedocs.io/en/2.6.1/get_started/introduction.html)

## Acknowledgements
AI was used to help generate some code snippets and functions, using Github Copilot. Any code generated by AI is noted in the comments of the relevant files. The rest of the code was written by me. The AI suggestions were reviewed and modified as necessary to ensure correctness and suitability and mainly for the dataset loading function. Any other uses were for simple syntax errors or small logical bugs that I needed fresh eyes for. All logic and structure of the code was my own.